...
5. **social_optimum.py:** Finds the social optimum of the graph, given the nuber of vehicles, the starting node, and the ending node. Very similar in structure to `nash_equilibrium.py`, with the only real difference being the actual calculation of the social optimum in line 66 and the positioning of error handling code.
6.**plot.py:** Creates a visualization of the given graph, including its attributes, i.e. node labels, polynomial labels for each edge (in the form ax + b), and arrows to indicate direction.
